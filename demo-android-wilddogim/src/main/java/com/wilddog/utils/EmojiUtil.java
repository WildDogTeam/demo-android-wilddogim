...
import com.wilddog.views.emoji.Emojicon;
import com.wilddog.views.emoji.People;
...
public class EmojiUtil {
	public HashMap<String,String> mEmojis = new HashMap<String, String>();
	...
	public void initData(){
		List<Emojicon> mData= Arrays.asList(People.DATA);
		mEmojiPageList.clear();
		/*for(int i=0 ;i<mData.size();i++){
			Emojicon icon=mData.get(i);
			mEmojis.put(String.valueOf(icon.getValue()),icon.getEmoji());
		}*/
...
/*		mEmojis.put("[龇牙笑]", R.drawable.f_static_000);
		...
		int pageCount = (int) Math.ceil(mData.size() / 27 + 0.1);
		...
		for (int iPage = 0; iPage < pageCount; iPage++) {
			...
			int mapNum=0;
			for (Emojicon emojicon: mData){
				if (mapNum>=startIndex){
					list.add(emojicon.getEmoji());
			...
			/*for (Map.Entry<String,String> entry : mEmojis.entrySet()) {
				if(mapNum>=startIndex){					
	...
	public SpannableString addEmoji(Context context,String text) {
		...
		int imageID = R.drawable.f_static_000;
	...
	private void replaceImage(Context context,SpannableString spannableString, Pattern patten, int start)
		...
		while (matcher.find()) {
			...
			Integer imageID = R.drawable.f_static_000;
